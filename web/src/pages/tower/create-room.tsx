        ...
        <Card className="w-full max-w-md min-w-[28rem] overflow-hidden bg-slate-900/90 border-purple-500/50 border shadow-xl shadow-purple-500/10 rounded-xl">
            <CardHeader className="pb-0">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <CardTitle className="text-xl font-bold text-white">Create Instance</CardTitle>
                        <CardDescription className="text-purple-300/80">Configure your battle parameters</CardDescription>
                    </div> 
                </div>
            </CardHeader>
            ...
            <CardContent className="space-y-6 pt-6">
                <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                        <Label htmlFor="enemy" className="text-sm font-medium text-slate-300">Enemy Type</Label>
                        <Select 
                            defaultValue="ai-1" 
                            onValueChange={(value)=>setRoomData({...roomData, enemyId: Number(value) })}
                        >
                            ...
                            <SelectContent className="bg-slate-800 border-slate-700">
                                {mobsData?.mobs?.map((mob: any)=> (
                                    <SelectItem value={mob?.id} className='text-white'>{mob?.name}</SelectItem>
                                ))} 
                            </SelectContent>
                        </Select> 
                    </div>
                    ...
                    <div className="space-y-1.5">
                        <Label htmlFor="maxMoves" className="text-sm font-medium text-slate-300">Max Moves</Label>
                        <Input
                            id="maxMoves"
                            type="number"
                            defaultValue="6"
                            ...
                            onChange={(e)=>setRoomData({...roomData, maxMoves: e.target.value })}
                        />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                        <Label htmlFor="rounds" className="text-sm font-medium text-slate-300">Rounds</Label>
                        <Input
                            id="rounds"
                            type="number"
                            defaultValue="5"
                            min="1"
                            className="bg-slate-800/80 border-slate-700/50 text-slate-200"
                            onChange={(e)=>setRoomData({...roomData, rounds: e.target.value })}
                        />
                    </div>
                    
                    <div className="space-y-1.5 flex flex-col justify-between">
                        <Label htmlFor="vsAi" className="text-sm font-medium text-slate-300">VS AI</Label>
                        <div className="flex items-center justify-between py-1">
                            <p className="text-xs text-slate-400">Compete against AI</p>
                            <Switch 
                                id="vsAi" 
                                defaultChecked 
                                onCheckedChange={(checked) => setRoomData({...roomData, vsAi: checked})}
                ...
                <Alert className="bg-indigo-900/20 border-indigo-500/30 text-indigo-200">
                    ...
                    <AlertDescription>
                        These settings determine the battle difficulty and length
                    </AlertDescription>
                </Alert>
            </CardContent>
            ...
            <CardFooter className="flex flex-col space-y-3 pt-2 pb-6">
                ...
                <div className="text-sm text-center text-slate-400 pt-1">
                    ...
                    <a href="#" className="text-purple-400 hover:text-purple-300 font-medium underline-offset-2 hover:underline transition-all">
                        Browse instances
                    </a>
                </div>
            </CardFooter>
