...
export const TeacherQuestionnaire = ({
    selectedLesson,
    selectedLessonItem,
    onPressNextLesson,
    isLastLessonItem,
    ...
    const {data: questionnaireAnswer} = useQuery({
        queryKey: ['teacher-questionnaire-answer', currentQuestion?.id, studentId],
        queryFn: async() => {
            if (currentQuestion?.id) {
                return await fetchQuestionAnswer(currentQuestion.id, studentId);
            }
        },
    ...
    return (
        <div className="pt-[38px]  w-full animate-fadeInUp  px-24">
            <div className={`flex  ${isFirstLessonItem?'justify-end':'justify-between'}`}>
                {(Number(currentQuestion?.ordinalNumber ?? 0) <= 1)
                ?
                <Button
